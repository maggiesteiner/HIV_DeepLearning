...
for (i in 1:length(list_names)){
  name = list_names[[i]]
  plot.new()
  file1 = read.csv(paste(name,".fastaroc.1.brnn.csv",sep=""))
  x1<-roc(data=file1,response="validation_labels_k",predictor="X.1")
  #x1<-ggroc(x)
  
  file2 = read.csv(paste(name,".fastaroc.2.brnn.csv",sep=""))
  x2<-roc(data=file2,response="validation_labels_k",predictor="X.1")
  #x2<-ggroc(x2)
  
  file3 = read.csv(paste(name,".fastaroc.3.brnn.csv",sep=""))
  x3<-roc(data=file3,response="validation_labels_k",predictor="X.1")
  #x3<-ggroc(x3)
  
  file4 = read.csv(paste(name,".fastaroc.4.brnn.csv",sep=""))
  x4<-roc(data=file4,response="validation_labels_k",predictor="X.1")
  #x4<-ggroc(x4)
  
  file5 = read.csv(paste(name,".fastaroc.5.brnn.csv",sep=""))
  ...
  auc_df_brnn[i,1]<-mean_auc
  auc_df_brnn[i,2]<-std_auc
  # if (k == 6){
  #   add_val=TRUE
  #   #average ROC curve
  # }
...
# 
# file = read.csv(paste(name,".fastaroc.2.brnn.csv",sep=""))
# roc(data=file,response="validation_labels_k",predictor="X.1",plot=TRUE,auc=TRUE,print.auc=FALSE,add=TRUE)
# file = read.csv(paste(name,".fastaroc.3.brnn.csv",sep=""))
# roc(data=file,response="validation_labels_k",predictor="X.1",plot=TRUE,auc=TRUE,print.auc=FALSE,add=TRUE)
# file = read.csv(paste(name,".fastaroc.4.brnn.csv",sep=""))
# roc(data=file,response="validation_labels_k",predictor="X.1",plot=TRUE,auc=TRUE,print.auc=FALSE,add=TRUE)
# file = read.csv(paste(name,".fastaroc.5.brnn.csv",sep=""))
# roc(data=file,response="validation_labels_k",predictor="X.1",plot=TRUE,auc=TRUE,print.auc=FALSE,add=TRUE)